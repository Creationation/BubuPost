...
export function BadgeStatut({
  ...
  const { dernierPassage, intervalleMinutes } = useScheduler()
  const etat = decrireAttente(scheduledAt, status, seconde * 1000, dernierPassage, intervalleMinutes * 60)
...
export function ProchainePublication({
  ...
  const etat = decrireAttente(suivante.scheduled_at, suivante.status, maintenant, dernierPassage, intervalleMinutes * 60)
  if (etat.phase === 'aucune') return null
